...
def address():
    ...
    response.title = T('Hashtag Grandma | Address')
    ...
    if digital_form.process().accepted:
        ...
        session.flash = T('Thanks!')
        redirect(URL('create', 'rule_type'))
    elif digital_form.errors:
        response.flash = T('Please fix any errors and try again')
...
def address_m():
    ...
    if digital_form.process().accepted:
        ...
        session.address_id = digital_form.vars.id
        session.flash = T('Thanks!')
        redirect(URL('create', 'rule_type_m'))
    elif digital_form.errors:
        response.flash = T('Please fix any errors and try again')
...
@auth.requires_login()
def set_rule():
    ''' Logs user's response as to whether they wish to use a keyword or send all photos '''
    response.title = T('Hashtag Grandma | Set Rule')

    rule_type = request.args(0, cast=int)
    inModal = 'modal' in request.get_vars
    if rule_type == 0:
        # Not using keyword
